...
class Window:
    def __init__(self,wireframe=True):
        self.running = True
        self.wireframe = wireframe
        self.interval = pi/1000          #amount the angles change when you move
    ...
    def draw_faces(self):
        for obj in self.space.objs:
            for face in obj.faces:
                face.points2pixels()
                py.draw.polygon(self.background,face.colour,[self.recenter_coord(point) for point in face.pixel_list])
    ...
    def change_angle(self):
        ...
        if not(self.wireframe):
            for obj in self.space.objs:
                obj.faces = sorted(obj.faces, key=lambda face:self.space.plane.center_point.dist(face.center), reverse=True)



if __name__ == "__main__":
    py.init()
    win = Window(wireframe=False)
